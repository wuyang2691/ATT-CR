...
import rasterio
import basicsr.utils.feature_detectors as  feature_detectors 
import torch
#S1波段定义
class S1Bands(Enum):
    VV = 1
...
class SEN12MSCRDataset(data.Dataset):
    ...
    def __init__(self, opt, scale=2000,data_augmentation=False,random_crop=False):
        
        self.scale=scale  #光学图像缩小多少倍 img=img/scale
        self.base_dir = opt['dataroot_gt'] #  base_dir #
    ...
    def get_scene_ids(self, season):
        ...
        scene_list = [os.path.basename(s)
                      for s in glob(os.path.join(path, "*")) if "s2_cloudy"  in s]#找到该目录下面的所有文件
        scene_list = [int(s.split('_')[-1]) for s in scene_list]#取数字部分

        # scene_list2 = [os.path.basename(s)
        #               for s in glob(os.path.join(path, "*")) if "s2_cloudy"  not in s]
        # scene_list2 = [int(s.split('_')[1]) for s in scene_list2]#取数字部分
        # scene_list2 = set(scene_list2)
    ...
    def get_patch(self, season, scene_id, patch_id, bands, iscloudimg):
        ...
        if not iscloudimg:
            ...
            self.gt_patch_path_out = patch_path
            #print(patch_path)
        else:
            ...
            patch_path = os.path.join(self.base_dir, season, scene, filename)
            #print(patch_path)
            self.cloudy_patch_path_out = patch_path
    ...
    def __getitem__(self,index):
        ...
        s2img = torch.from_numpy(s2img)
        return {
            'lq': inputdata,
            'gt': s2img,
            'lq_path': self.cloudy_patch_path_out,
            'gt_path': self.gt_patch_path_out
        }
...
#测试
# dataset=SEN12MSCRDataset("/data/wy/data/cloud/SEN12MS-CR/dsen2cr-torch-h/train")
