...
def collect_metadata():
    meta = {}
    with open(path.join("./causal_product", "__init__.py")) as f:
        lines = iter(f)
...
def get_extensions():
    extensions = [
        ...
        CppExtension(
            "causal_product.causal_product_cpu",
            sources=[
                "causal_product/causal_product_cpu.cpp"
            ],
    ...
    if cuda_toolkit_available():
        ...
        extensions += [
            CUDAExtension(
                "causal_product.causal_product_cuda",
                sources=[
                    "causal_product/causal_product_cuda.cu"
                ],
...
def setup_package():
    ...
    version_suffix = os.getenv("causal_product_VERSION_SUFFIX", "")
    setup(
        name="causal_product",
